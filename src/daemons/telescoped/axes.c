 ...
 */
int axis_home(MotorInfo *mip, FifoId fid, int first)
{
    if (virtual_mode)
    {
        vmcSetHome(mip->axis);
    ...
    else
    {
        ...
        if (first)
        {
            ...
            mjdto[i] = telstatshmp->now.n_mjd + 4.5 * (mip->poslim - mip->neglim) / mip->maxvel / SPD;
            /* seeks at half speed */
        }
        ...
        mjdto[i] = telstatshmp->now.n_mjd + 2 * (mip->poslim - mip->neglim) / mip->maxvel / SPD;
 ...
 */
int axisLimitCheck(MotorInfo *mip, char msgbuf[])
{
    if (virtual_mode)
    {
 ...
 */
int axisMotionCheck(MotorInfo *mip, char msgbuf[])
{
#if 0
	/* TODO */
 ...
 * return 1 if in-progress, 0 done, else -1.
 */
int axis_limits(MotorInfo *mip, FifoId fid, int first)
{
    ...
    else
    {
        ...
        if (!mip->havelim)
        {
            fifoWrite(fid, 0, "Axis %d: ok, but no limit switches are configured", mip->axis);
            return (-1);
        }
        ...
        {
            csiStop(mip, 1);
            fifoWrite(fid, -2, "Axis %d motion error finding limits: %s", axis, buf);
            return (-1);
        }
        ...
        else
        {
            ...
            mip->limiting = 0;
            csiStop(mip, 1);
            fifoWrite(fid, -5, "Axis %d: %c limit appears stuck on", axis, seeking[i]);
            return (-1);
        }
    }
}
