...
//		TRACE "vmcService %d. Tracking = %d\n",node,pvc->tracking);
	...
	if(active_func[node]) (*active_func[node])(node);
}
...
if(node == 2)	
    ...
    while(abs(position) > pvc->countsPerRev) {
        if(position < 0) { position += pvc->countsPerRev; }
        else { position -= pvc->countsPerRev; }
    }
...
int vmcSetTrackPath(int node, int num, int startMs, int ivalMs, double *path)
{
	...
	if(pvc->trackPath) free(pvc->trackPath); // free previous
	pvc->trackPath = malloc(num * sizeof(double));
	if(!pvc->trackPath) return -1;
...
static void oMotorGo(VCNodePtr pvc)
{
	...
	if(pvc->velocity != wantVel) pvc->clamped = 1;
	else						 pvc->clamped = 0;
