...
reopenPty (int cfd)
{
	...
	if (ptyfd != cfd) {
	    if (dup2 (ptyfd, cfd) < 0) {
		daemonLog ("reopenPty dup2(%d, %d): %s", ptyfd, cfd,
							    strerror(errno));
		exit(1);
	    }
	    close (ptyfd);
...
newSerial (CInfo *cip, int baud)
{
	...
	for (lcip = cinfo; lcip < &cinfo[NHOSTS]; lcip++) {
	    if (lcip != cip && lcip->inuse && lcip->toaddr == to
						&& lcip->why == FOR_SERIAL) {
 ...
 */
static int
readLANpacket(char *what, int nto, int fr)
{
	Byte d;
	int n;
	...
	rpktlen = 0;
		...
		default:			/* gathering data */
		    ...
		    if (rpktlen >= rpkt[PB_COUNT]+PB_DATA) { /* if have all */
...
static void
buildCtrlPkt (int from, int to, PktType t)
{
	xpkt[PB_SYNC] = PSYNC;
	xpkt[PB_TO] = to;
	xpkt[PB_FR] = from;
...
ack4xpkt (void)
{
	...
	if (t != PT_ACK) {
	     ...
	     */
	    if (HA2CIP(haddr)->why == FOR_BOOT
				    && (xpkt[PB_INFO]&PT_MASK) == PT_BOOTREC) {
	    ...
	    if (HA2CIP(haddr)->why == FOR_SHELL
				    && (xpkt[PB_INFO]&PT_MASK) == PT_INTR) {
		int cfd = HA2CFD (haddr);
		char zero = 0;
		if (verbose)
 ...
 */
void
sendPkt (Byte pkt[], int try)
{
	int npkt = pktSize(pkt);

	if (verbose) {
	    daemonLog("Sending %s packet: %d data from %d to %d seq 0x%x retry %d\n", 
		    p2tstr((Pkt*)pkt), pkt[PB_COUNT], pkt[PB_FR], pkt[PB_TO],
						pkt[PB_INFO]>>PSQ_SHIFT, try);
...
p2tstr (Pkt *pkp)
{
	...
	case PT_SHELL:
		...
		return ("LOGGING");
	    return ("SHELL");
	case PT_BOOTREC: return ("BOOTREC");
	case PT_INTR:    return ("INTR");
	case PT_KILL:    return ("KILL");
	case PT_ACK:     return ("ACK");
	case PT_REBOOT:  return ("REBOOT");
	case PT_PING:    return ("PING");
	case PT_GETVAR:  return ("GETVAR");
	case PT_SETVAR:  return ("SETVAR");
	case PT_SERDATA: return ("SERDATA");
	case PT_SERSETUP:return ("SERSETUP");
	default:         return ("???");
	}
}
...
{
	switch (why) {
	case FOR_SHELL:	 return ("FOR_SHELL");
	case FOR_BOOT:	 return ("FOR_BOOT");
	case FOR_REBOOT: return ("FOR_REBOOT");
	case FOR_SERIAL: return ("FOR_SERIAL");
	default:	 return ("FOR_???");
	}
}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: csimcd.c,v $ $Date: 2006/01/13 20:42:16 $ $Revision: 1.1.1.1 $ $Name:  $"};
