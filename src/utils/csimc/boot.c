...
static void guardFirmware(int signo)
{
 ...
 * return 0 if ok else -1.
 */
int loadFirmware(int addr, char *fn)
{
    void (*oldint)();
    FILE *ffp;
    char buf[128];
...
static int download(char *fn, FILE *fp, int fd)
{
    ...
    while (1)
    {
        ...
        if (ms >= 500)
        {
            ...
            int min = left / 60;
            int sec = left % 60;
            int n = printf("%3ld%% @ %5dB/s %3d:%02d", 100 * fnow / fsize, 1000 * totd / ms, min, sec);
            back[n] = 0;
            while (--n >= 0)
                back[n] = '\b';
            printf("%s", back);
