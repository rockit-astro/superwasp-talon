...
static void cmdInterrupt(void)
{
    ...
    if (csi_intr(cfd) < 0)
    {
        printf("Error sending INTR to %d: %s", csi_f2n(cfd), strerror(errno));
        closeFD(cfd);
    }
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: csimc.c,v $ $Date: 2006/01/13 20:42:15 $ $Revision: 1.1.1.1 $ $Name:  $"};
