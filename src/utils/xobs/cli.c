...
void initCli(void)
{
    ...
    /* Add input handler */
    srvid = XtAppAddInput(app, srvfd, (XtPointer)XtInputReadMask, srvinput_cb, (XtPointer)NULL);
    srvidon = 1;
}
...
void cli_add_write(struct cliclient *c)
{
    ...
    {
        c->writeid = XtAppAddInput(app, c->fd, (XtPointer)XtInputWriteMask, clioutput_cb, (XtPointer)c);
        c->writeidon = 1;
    }
}
...
static void srvinput_cb(XtPointer client_data, int *fd, XtInputId *id)
{
    ...
    c->readid = XtAppAddInput(app, clifd, (XtPointer)XtInputReadMask, cliinput_cb, (XtPointer)c);
    c->readidon = 1;
