...
static void kpEH(w, client, ep, dispatch) Widget w;
XtPointer client;
XEvent *ep;
Boolean *dispatch;
{
...
static void mkPadGUI()
{
    ...
    n++;
    XtSetArg(args[n], XmNtitle, "Software Hand Paddle");
    n++;
    ...
    n++;
    XtSetArg(args[n], XmNrightPosition, 45);
    n++;
    ...
    n++;
    XtSetArg(args[n], XmNtopWidget, arrowbox_w);
    n++;
    ...
    n++;
    XtSetArg(args[n], XmNbottomWidget, close_w);
    n++;
    XtSetArg(args[n], XmNleftAttachment, XmATTACH_POSITION);
    n++;
    XtSetArg(args[n], XmNleftPosition, 55);
    n++;
...
static Widget mkArrows(Widget p_w)
{
    ...
    n++;
    XtSetArg(args[n], XmNleftWidget, wl_w);
    n++;
    XtSetArg(args[n], XmNrightAttachment, XmATTACH_WIDGET);
    n++;
    XtSetArg(args[n], XmNrightWidget, el_w);
    n++;
    ...
    n = 0;
    XtSetArg(args[n], XmNwidth, 3 * ARROWSZ);
    n++;
    XtSetArg(args[n], XmNheight, 3 * ARROWSZ);
    n++;
    ...
    for (i = 0; i < XtNumber(arrows); i++)
    {
        ...
        n = 0;
        XtSetArg(args[n], XmNx, awp->x);
        n++;
        XtSetArg(args[n], XmNy, awp->y);
        n++;
        XtSetArg(args[n], XmNwidth, ARROWSZ);
        n++;
        XtSetArg(args[n], XmNheight, ARROWSZ);
        n++;
        XtSetArg(args[n], XmNarrowDirection, awp->dir);
        n++;
    ...
    XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
    n++;
    XtSetArg(args[n], XmNtopWidget, nl_w);
    n++;
    XtSetArg(args[n], XmNbottomAttachment, XmATTACH_WIDGET);
    n++;
    XtSetArg(args[n], XmNbottomWidget, sl_w);
    n++;
    XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
    n++;
    XtSetArg(args[n], XmNleftWidget, wl_w);
...
static Widget mkButtons(Widget p_w)
{
    ...
    for (i = 0; i < XtNumber(buttons); i++)
    {
        ...
        n++;
        XtSetArg(args[n], XmNtopPosition, bwp->pos);
        n++;
        XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
        n++;
...
static void armArrow(Widget w)
{
    if (XmToggleButtonGetState(focus_w))
    {
        if (w == n_w)
            fifoMsg(Focus_Id, "j-");
        if (w == s_w)
...
static void buttonCB(Widget w, XtPointer client, XtPointer call)
{
    ...
    if (w != coarse_w)
        XmToggleButtonSetState(coarse_w, False, True);
    if (w != fine_w)
        XmToggleButtonSetState(fine_w, False, True);
