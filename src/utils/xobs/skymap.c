...
mkSky (Widget main_w)
{
	...
	skyda_w = XtVaCreateManagedWidget ("SDA", xmDrawingAreaWidgetClass,fr_w,
...
showSkyMap()
{
	...
	if (moonobj.s_alt >= 0) {
	    ...
	    XFillArc(dsp, sky_pm, skyGC, x-MOONSZ/2, y-MOONSZ/2, MOONSZ,
							    MOONSZ, 0, 360*64);
	    XSetForeground (dsp, skyGC, sky_p);
	    XFillArc (dsp, sky_pm, skyGC, x-MOONSZ, y-MOONSZ/2, MOONSZ, MOONSZ,
								    0, 360*64);
	}
	...
	if (sunobj.s_alt >= 0) {
	    ...
	    XFillArc (dsp,  sky_pm,  skyGC,  x-SUNSZ/2, y-SUNSZ/2, SUNSZ,
							    SUNSZ, 0, 360*64);
	}
...
static void
skyExpCB (Widget w, XtPointer client, XtPointer call)
{
	if (!sky_pm) {
...
static void
aa2xy (double alt, double az, int *xp, int *yp)
{
	double tmp = PI/2 - alt;
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: skymap.c,v $ $Date: 2006/01/13 20:42:47 $ $Revision: 1.1.1.1 $ $Name:  $"};
