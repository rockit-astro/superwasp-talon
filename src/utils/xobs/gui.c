...
static Widget mkPrompt(Widget p_w, int tc, Widget *l_wp, Widget *tf_wp, Widget *lb_wp);
static void mkRow(Widget p_w, Widget w_p[], int n, int w);
static void mkFrame(Widget parent_w, char *name, char *title, Widget *frame_wp, Widget *form_wp);
...
void mkGUI(char *version)
{
    ...
    main_w = XtVaCreateManagedWidget("MainF", xmFormWidgetClass, toplevel_w, XmNverticalSpacing, 10,
                                     XmNhorizontalSpacing, 10, XmNresizePolicy, XmRESIZE_NONE, NULL);

    skymap_w = mkSky(main_w);
    XtVaSetValues(skymap_w, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, NULL);

    cur_w = mkCurrent(main_w);
    XtVaSetValues(cur_w, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, skymap_w,
                  XmNrightAttachment, XmATTACH_FORM, NULL);

    hf_w = XtVaCreateManagedWidget("HF", xmFormWidgetClass, main_w, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                                   skymap_w, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, NULL);

    ctrl_w = mkControl(hf_w);
    XtVaSetValues(ctrl_w, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment,
                  XmATTACH_FORM, NULL);
    ...
    msgf_w = mkMsg(main_w);
    XtVaSetValues(msgf_w, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, info_w, XmNbottomAttachment, XmATTACH_FORM,
 ...
 */
int setColor(Widget w, char *resource, Pixel newp)
{
    Pixel oldp;
...
Widget mkLight(Widget p_w)
{
    ...
    da_w = XtVaCreateManagedWidget("LDA", xmDrawingAreaWidgetClass, p_w, XmNwidth, LIGHTW + 8, XmNheight, LIGHTH + 8,
                                   NULL);

    /* get shadow colors for the current background */
    XtVaGetValues(da_w, XmNcolormap, &cm, XmNbackground, &bg_p, NULL);
    XmGetColors(DefaultScreenOfDisplay(dsp), cm, bg_p, &fg_p, &tshadow_p, &bshadow_p, &sel_p);
...
static Widget mkCurrent(Widget main_w)
{
    ...
    static Ctrl ctrls[] = {
        {""}, {"RA(J2000)"}, {"Dec(J2000)"}, {"HA"}, {"Altitude"}, {"Azimuth"},
    };
    Widget fr_w, f_w;
    Widget lf_w;
    ...
    lf_w = XtVaCreateManagedWidget("CLF", xmFormWidgetClass, f_w, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment,
                                   XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, NULL);
    for (i = 0; i < XtNumber(l_w); i++)
    {
        l_w[i] = XtVaCreateManagedWidget("CL", xmLabelWidgetClass, lf_w, NULL);
        wltprintf(prT, l_w[i], "%s", ctrls[i].lbl);
    }
    mkRow(lf_w, l_w, XtNumber(l_w), 20);

    lf_w = XtVaCreateManagedWidget("CCF", xmFormWidgetClass, f_w, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, lf_w,
                                   XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, NULL);
    l_w[0] = XtVaCreateManagedWidget("CL", xmLabelWidgetClass, lf_w, XmNalignment, XmALIGNMENT_CENTER, NULL);
    wltprintf(prT, l_w[0], "Current");
    for (i = 1; i < XtNumber(l_w); i++)
    {
        l_w[i] = XtVaCreateManagedWidget("CPT", xmTextFieldWidgetClass, lf_w, XmNbackground, uneditableColor,
                                         XmNcolumns, 10, XmNcursorPositionVisible, False, XmNeditable, False,
...
static Widget mkControl(Widget main_w)
{
    ...
    static Ctrl ctrls[] = {
        {"Stop", 0, g_stop, &g_w[CSTOP_W], "Stop all equipment"},
    ...
    tbl_w = XtVaCreateManagedWidget("CRC", xmRowColumnWidgetClass, f_w, XmNtopAttachment, XmATTACH_FORM,
                                    XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM,
                                    XmNrightAttachment, XmATTACH_FORM, XmNnumColumns, (XtNumber(ctrls) + 1) / 2,
                                    XmNadjustLast, False, XmNisAligned, False, XmNorientation, XmHORIZONTAL, XmNpacking,
                                    XmPACK_COLUMN, XmNspacing, 5, NULL);
    ...
    for (i = 0; i < XtNumber(ctrls); i++)
    {
        ...
        if (cp->tb)
        {
            w = XtVaCreateManagedWidget("T", xmToggleButtonWidgetClass, tbl_w, XmNalignment, XmALIGNMENT_BEGINNING,
                                        XmNindicatorOn, True, XmNindicatorType, XmN_OF_MANY, XmNvisibleWhenOff, True,
        ...
        {
            w = XtVaCreateManagedWidget("P", xmPushButtonWidgetClass, tbl_w, XmNentryAlignment, XmALIGNMENT_CENTER,
                                        XmNmarginHeight, 0, NULL);
            if (strcmp(cp->lbl, "Stop") == 0)
                XtVaSetValues(w, XmNforeground, getColor(toplevel_w, "white"), XmNbackground,
                              getColor(toplevel_w, "red"), NULL);
            if (cp->cb)
...
static Widget mkStatus(Widget main_w)
{
    ...
    static Ctrl ctrls[] = {
        {"Tracking", &g_w[STLT_W]}, {"Slewing", &g_w[SSLT_W]}, {"Homing", &g_w[SHLT_W]},
        {"Limiting", &g_w[SLLT_W]}, {"Confirm", &g_w[SCLT_W]},
    };
    Widget fr_w, f_w;
    Widget tbl_w;
    int i;
    ...
    tbl_w = XtVaCreateManagedWidget("CRC", xmRowColumnWidgetClass, f_w, XmNtopAttachment, XmATTACH_FORM,
                                    XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM,
                                    XmNrightAttachment, XmATTACH_FORM, XmNpacking, XmPACK_TIGHT, XmNspacing, 4, NULL);
    ...
    for (i = 0; i < XtNumber(ctrls); i++)
    {
        ...
        sf_w = XtVaCreateManagedWidget("SF", xmFormWidgetClass, tbl_w, NULL);

        lt_w = mkLight(sf_w);
        XtVaSetValues(lt_w, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, XmNrightAttachment,
                      XmATTACH_FORM, NULL);
        ...
        sl_w =
            XtVaCreateManagedWidget("P", xmLabelWidgetClass, sf_w, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment,
                                    XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment,
                                    XmATTACH_WIDGET, XmNrightWidget, lt_w, XmNalignment, XmALIGNMENT_BEGINNING, NULL);
...
static Widget mkScope(Widget main_w)
{
    ...
    rrc_w = XtVaCreateManagedWidget("SRC", xmRowColumnWidgetClass, f_w, XmNtopAttachment, XmATTACH_FORM,
                                    XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM,
                                    XmNrightAttachment, XmATTACH_FORM, XmNpacking, XmPACK_TIGHT, XmNspacing, 6, NULL);
    ...
    for (i = 0; i < XtNumber(ctrls); i++)
    {
        ...
        XtVaSetValues(tf_w, XmNbackground, editableColor, XmNpendingDelete, False, XmNmaxLength, 9, NULL);
        if (ctrls[i].tip)
...
static void msg_erase(Widget w, XtPointer client, XtPointer call)
{
...
static void msg_latest(Widget w, XtPointer client, XtPointer call)
{
...
static Widget mkMsg(Widget main_w)
{
    ...
    e_w = XtVaCreateManagedWidget("E", xmPushButtonWidgetClass, f_w, XmNtopAttachment, XmATTACH_FORM,
                                  XmNrightAttachment, XmATTACH_FORM, NULL);
    wltprintf(pbT, e_w, "Erase");
    XtAddCallback(e_w, XmNactivateCallback, msg_erase, NULL);
    wtip(e_w, "Erase all messages");

    l_w = XtVaCreateManagedWidget("L", xmPushButtonWidgetClass, f_w, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                                  e_w, XmNtopOffset, 5, XmNleftAttachment, XmATTACH_OPPOSITE_WIDGET, XmNleftWidget, e_w,
                                  XmNrightAttachment, XmATTACH_FORM, NULL);
    ...
    sw_w = XtParent(msg_w);
    XtVaSetValues(sw_w, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment,
                  XmATTACH_FORM, XmNrightAttachment, XmATTACH_WIDGET, XmNrightWidget, e_w, XmNrightOffset, 5, NULL);
    XtVaSetValues(msg_w, XmNeditMode, XmMULTI_LINE_EDIT, XmNeditable, False, XmNcursorPositionVisible, False, XmNrows,
                  10, XmNmarginHeight, 1, XmNmarginWidth, 1, NULL);
...
static Widget mkInfo(Widget main_w)
{
    ...
    lf_w = XtVaCreateManagedWidget("ILF", xmFormWidgetClass, f_w, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment,
                                   XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, NULL);
    for (i = 0; i < XtNumber(l_w1); i++)
    {
        l_w1[i] = XtVaCreateManagedWidget("IL", xmLabelWidgetClass, lf_w, NULL);
        wltprintf(prT, l_w1[i], "%s", ctrls1[i].lbl);
    }
    mkRow(lf_w, l_w1, XtNumber(l_w1), 25);

    lf_w = XtVaCreateManagedWidget("ILF", xmFormWidgetClass, f_w, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, lf_w,
                                   XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, NULL);
    for (i = 0; i < XtNumber(l_w1); i++)
    {
        l_w1[i] = XtVaCreateManagedWidget("IL", xmTextFieldWidgetClass, lf_w, XmNbackground, uneditableColor,
                                          XmNcolumns, 5, XmNcursorPositionVisible, False, XmNeditable, False,
                                          XmNmarginHeight, 1, XmNmarginWidth, 1, NULL);
        if (ctrls1[i].wp)
            *ctrls1[i].wp = l_w1[i];
        if (ctrls1[i].tip)
    ...
    sep_w = XtVaCreateManagedWidget("S", xmSeparatorWidgetClass, f_w, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                                    lf_w, XmNtopOffset, 10, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment,
                                    XmATTACH_FORM, NULL);

    lf_w = XtVaCreateManagedWidget("ILF", xmFormWidgetClass, f_w, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
                                   sep_w, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, NULL);
    for (i = 0; i < XtNumber(l_w2); i++)
    {
        l_w2[i] = XtVaCreateManagedWidget("IL", xmLabelWidgetClass, lf_w, NULL);
    ...
    for (i = 0; i < XtNumber(l_w2); i++)
    {
        l_w2[i] = XtVaCreateManagedWidget("IL", xmTextFieldWidgetClass, lf_w, XmNbackground, uneditableColor,
                                          XmNcolumns, 5, XmNcursorPositionVisible, False, XmNeditable, False,
                                          XmNmarginHeight, 1, XmNmarginWidth, 1, NULL);
        if (ctrls2[i].wp)
            *ctrls2[i].wp = l_w2[i];
...
static void mkFrame(Widget parent_w, char *name, char *title, Widget *frame_wp, Widget *form_wp)
{
    ...
    fr_w = XtVaCreateManagedWidget(name, xmFrameWidgetClass, parent_w, NULL);

    title_w = XtVaCreateManagedWidget("FL", xmLabelWidgetClass, fr_w, XmNchildType, XmFRAME_TITLE_CHILD,
                                      XmNchildHorizontalAlignment, XmALIGNMENT_CENTER, NULL);
    wltprintf(frT, title_w, "%s", title);

    f_w = XtVaCreateManagedWidget("FF", xmFormWidgetClass, fr_w, XmNchildType, XmFRAME_WORKAREA_CHILD, NULL);
...
static void mkRow(Widget f_w, Widget w_p[], int n, int w)
{
    int i;
    ...
    for (i = 0; i < n; i++)
        XtVaSetValues(w_p[i], XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment,
                      XmATTACH_POSITION, XmNleftPosition, i * (w + 1), XmNrightAttachment, XmATTACH_POSITION,
                      XmNrightPosition, w + i * (w + 1), NULL);
}
...
static Widget mkPrompt(Widget p_w, int tc, Widget *l_wp, Widget *tf_wp, Widget *lb_wp)
{
    ...
    tf_w = XtVaCreateManagedWidget("PTF", xmTextFieldWidgetClass, f_w, XmNtopAttachment, XmATTACH_FORM,
                                   XmNbottomAttachment, XmATTACH_FORM, XmNcolumns, tc, XmNmarginHeight, 1,
                                   XmNmarginWidth, 1, NULL);

    if (lb_wp)
    {
        Widget lb_w = mkLight(f_w);

        XtVaSetValues(lb_w, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, XmNrightAttachment,
                      XmATTACH_FORM, NULL);

        XtVaSetValues(tf_w, XmNrightAttachment, XmATTACH_WIDGET, XmNrightWidget, lb_w, XmNrightOffset, 10, NULL);

        *lb_wp = lb_w;
    }
    else
    {
        XtVaSetValues(tf_w, XmNrightAttachment, XmATTACH_FORM, NULL);
    }

    l_w = XtVaCreateManagedWidget("PL", xmLabelWidgetClass, f_w, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment,
                                  XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_WIDGET,
                                  XmNrightWidget, tf_w, NULL);
...
static void drawLt(Widget wid)
{
    ...
    XtVaGetValues(wid, XmNforeground, &p, XmNwidth, &w, XmNheight, &h, NULL);
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: gui.c,v $ $Date: 2006/01/13 20:42:47 $ $Revision: 1.1.1.1 $ $Name:  $"};
