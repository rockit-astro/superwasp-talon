...
int docommand(struct cliclient *c, unsigned char cmd, int paylen, unsigned char *paybuf, char *errstr)
{
    ...
    else if (cmd == TCSCMD_STOP_DOME || cmd == TCSCMD_DOME || cmd == TCSCMD_SET_ALARM)
    {
        /* Dome */
    ...
    case TCSCMD_SLEW_OBJECT:
        ...
        else
        {
            ...
            else if (!strcasecmp(objname, "zenith") || !strcasecmp(objname, "service"))
            {
    ...
    case TCSCMD_SLEW_RADEC:
        ...
        {
            cmd_err(errbuf, "incorrect payload size, expected %d, got %d", sizeof(struct cli_object), paylen);
            rv = TCSRET_USAGE;
        }
        else
        {
    ...
    case TCSCMD_SLEW_ALTAZ:
        ...
        {
            cmd_err(errbuf, "incorrect payload size, expected %d, got %d", sizeof(off), paylen);
            rv = TCSRET_USAGE;
        }
        else
        {
    ...
    case TCSCMD_TRACK_RADEC:
        ...
        else
        {
            ...
            {
                if (fifoMsg(Tel_Id, "RA:%.6f Dec:%.6f Epoch:%g", obj.ra, obj.dec, obj.equinox) == -1)
                {
                    cmd_err(errbuf, "telescope not available");
                    rv = TCSRET_FAILED;
                }
    ...
    case TCSCMD_OFFSET_RADEC:
        ...
        else
        {
            ...
            if (telstatshmp->telstate == TS_HUNTING || telstatshmp->telstate == TS_TRACKING)
            {
    ...
    case TCSCMD_ENGMODE:
        ...
        {
            cmd_err(errbuf, "incorrect payload size, expected %d, got %d", sizeof(int), paylen);
            rv = TCSRET_USAGE;
        }
        else
        {
    ...
    case TCSCMD_HOME:
        ...
        {
            cmd_err(errbuf, "incorrect payload size, expected %d, got %d", sizeof(unsigned char), paylen);
            rv = TCSRET_USAGE;
        }
        else
        {
...
void check_tel_reply(int rv, char *buf)
{
    ...
    if (current_tel_cmd > 0)
    {
        ...
        else if (rv == 0)
        {
            ...
            if ((current_tel_cmd == TCSCMD_SLEW_OBJECT || current_tel_cmd == TCSCMD_SLEW_RADEC ||
                 current_tel_cmd == TCSCMD_SLEW_ALTAZ || current_tel_cmd == TCSCMD_OFFSET_RADEC ||
                 current_tel_cmd == TCSCMD_OFFSET_ALTAZ) &&
                !strncasecmp(buf, "stop", 4))
                /* Not ours */
                ;
            else
            {
...
void cli_move_telescope(void)
{
    ...
    if (current_tel_cmd > 0)
    {
        ...
        if (replycommand(current_tel_client, current_tel_cmd, TCSRET_FAILED, "GUI override", errstr))
            fatal(1, "replycommand: %s", errstr);
...
void cli_move_dome(void)
{
    ...
    if (current_dome_cmd > 0)
    {
        ...
        if (replycommand(current_dome_client, current_dome_cmd, TCSRET_FAILED, "GUI override", errstr))
            fatal(1, "replycommand: %s", errstr);
