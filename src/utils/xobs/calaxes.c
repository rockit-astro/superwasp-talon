...
static char blank[] = " ";
static char intro_msg[] = "Select a star with Sky View, center with Paddle, Mark.";
...
int axes_xephemSet(char *buf)
{
    ...
    wlprintf(msg_w, "Center %s using Paddle, then Mark; or reselect.", oip->obj.o_name);
    nobjs = newi + 1;
    return (-1);
}

/* build the user interface */
static void mkCalGUI()
{
    typedef struct
    {
        char *name;
    ...
    n = 0;
    XtSetArg(args[n], XmNhorizontalSpacing, 10);
    n++;
    XtSetArg(args[n], XmNverticalSpacing, 10);
    n++;
    XtSetArg(args[n], XmNresizePolicy, XmRESIZE_GROW);
    n++;
    XtSetArg(args[n], XmNfractionBase, 3 * XtNumber(ctrls) + 1);
    n++;
    XtSetArg(args[n], XmNautoUnmanage, False);
    n++;
    XtSetArg(args[n], XmNtitle, "Axis Calibration");
    n++;
    ...
    wlprintf(w, "Solution:");
    wtip(w, "This area shows the current candidate telescope axis solution");
    XtManageChild(w);
...
static void mkTable(Widget rc_w)
{
    Arg args[20];
    Widget f_w;
    Widget w;
    ...
    XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNleftOffset, CHCINDENT);
    n++;
...
static void mkRow(Widget p_w, int i)
{
    ...
    n++;
    XtSetArg(args[n], XmNrightWidget, l_w);
    n++;
    XtSetArg(args[n], XmNrightOffset, 6);
    n++;
    ...
    n++;
    XtSetArg(args[n], XmNleftWidget, l_w);
    n++;
    XtSetArg(args[n], XmNleftOffset, 6);
    n++;
...
static void undoCB(Widget w, XtPointer client, XtPointer call)
{
    if (nobjs > 0)
    {
        ...
        if (oip->marked)
        {
            ...
            wlprintf(msg_w, "Redo %s, reselect, or Undo more.", oip->obj.o_name);
        }
        else
        {
            ...
            case 1:
                ...
                ;
            case 2:
                wlprintf(msg_w, "2 stars are marked. More is better, but can Solve now.");
                break;
                ;
            default:
                wlprintf(msg_w, "%d stars are marked. Do another or Solve.", nobjs);
                break;
                ;
            }
        }
...
static void markCB(Widget w, XtPointer client, XtPointer call)
{
    ...
    if (nobjs == 0)
    {
        wlprintf(msg_w, "Nothing to mark.. first select and center a star.");
        return;
    }
    ...
    if (nobjs < 2)
        ...
        wlprintf(msg_w, "%s is marked. More is better, but can Solve now.", oname);
...
static void installCB(Widget w, XtPointer client, XtPointer call)
{
    ...
    if (installNew() == 0)
    {
        ...
        msg("Installed new calibration model: %g %g %g %g %g", tax.HT, tax.DT, tax.XP, tax.YC, tax.NP);
    }
}
...
static void showSolution(int empty)
{
    ...
    wlprintf(new0_w, "  T Pole :     HA =%s  Dec = %s  NonP = %s", htbuf, dtbuf, npbuf);
 ...
 * order
 */
static int resid_qf(const void *p1, const void *p2)
{
...
static void solveNew()
{
    ...
    /* dump for posterity */
    time(&t);
    printf("H/D/X/Y, suitable for use with mntmodel, at UT %s", asctime(gmtime(&t)));
    for (i = 0; i < nobjs; i++)
    {
        ObjInfo *oip = &objs[i];
        printf("%10.7f %10.7f %10.7f %10.7f %s\n", oip->ha, oip->dec, oip->henc, oip->denc, oip->obj.o_name);
    }
    ...
    if (s < 0)
    {
        wlprintf(msg_w, "%d-star solution failed... see xobs.log for raw values.", nobjs);
        return;
    }
    ...
    /* suggest next step */
    if (nobjs < MAXOBJS)
        wlprintf(msg_w, "%d-star solution... add more, Undo, or Install if like.", nobjs);
    else
        wlprintf(msg_w, "%d-star solution... Undo, or Install if like.", nobjs);
}
