...
void g_stop(Widget w, XtPointer client, XtPointer call)
{
...
void g_exit(Widget w, XtPointer client, XtPointer call)
{
...
void g_home(Widget w, XtPointer client, XtPointer call)
{
...
void g_limit(Widget w, XtPointer client, XtPointer call)
{
    /* create both for hlsetup() */
    if (!limit_w)
        limitCreateW();
...
static void homeCreateW()
{
    ...
    XtVaSetValues(home_w, XmNverticalSpacing, 10, XmNhorizontalSpacing, 10, XmNautoUnmanage, False, NULL);
    XtVaSetValues(XtParent(home_w), XmNtitle, "Find Homes", NULL);

    /* put a rc in it */
    rc_w = XmCreateRowColumn(home_w, "HRC", NULL, 0);
    XtManageChild(rc_w);
    XtVaSetValues(rc_w, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment,
                  XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNisAligned, True, XmNentryAlignment,
    ...
    XtVaSetValues(w, XmNheight, 10, XmNseparatorType, XmNO_LINE, NULL);
    XtManageChild(w);
    w = XmCreatePushButton(rc_w, "PB", NULL, 0);
...
static void limitCreateW()
{
    ...
    XtVaSetValues(limit_w, XmNverticalSpacing, 10, XmNhorizontalSpacing, 10, XmNautoUnmanage, False, NULL);
    XtVaSetValues(XtParent(limit_w), XmNtitle, "Find Limits", NULL);
...
static void homeCB(Widget w, XtPointer client, XtPointer call)
{
    ...
    for (i = 0; i < N_AX; i++)
    {
        ...
        if ((idmask & (1 << i)) && XtIsSensitive(hip->hpbw))
        { /*lazy "have"*/
            if (hip->telaxcode)
...
static void limitCB(Widget w, XtPointer client, XtPointer call)
{
    ...
    {
        HLInfo *hip = &hl_info[i];
        if ((idmask & (1 << i)) && XtIsSensitive(hip->lpbw))
        { /*lazy "have"*/
            if (hip->telaxcode)
                axcodes[naxcodes++] = hip->telaxcode;
            else
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: control.c,v $ $Date: 2006/01/13 20:42:46 $ $Revision: 1.1.1.1 $ $Name:  $"};
