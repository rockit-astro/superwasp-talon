...
#ifdef FAKE_RUN
    nimg = 5;
    imghist[1].pos = 300; imghist[1].sd = 29.7;
    imghist[2].pos = 400; imghist[2].sd = 12.6;
    imghist[0].pos = 200; imghist[0].sd = 10.5;
    imghist[3].pos = 350; imghist[3].sd = 24.4;
    imghist[4].pos = 325; imghist[4].sd = 33.3;
...
installCB (Widget w, XtPointer client, XtPointer call)
{
	...
	maxi = 0;
	for (i = 1; i < nimg; i++)
	    if (imghist[i].sd > imghist[maxi].sd)
		maxi = i;
	p = imghist[maxi].pos;
...
/* REMOVED -- new focustemp function replaces this
...
findNew()
{
	...
	besti = 0;
	for (i = 1; i < nimg; i++)
	    if (imghist[i].sd > imghist[besti].sd)
		besti = i;
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: autofocus.c,v $ $Date: 2006/01/13 20:42:46 $ $Revision: 1.1.1.1 $ $Name:  $"};
