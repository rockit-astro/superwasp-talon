...
void zero_mem(loc, len) void *loc;
unsigned len;
{
 ...
 */
int tickmarks(min, max, numdiv, ticks) double min, max;
int numdiv;
double ticks[];
{
 ...
 */
char *obj_description(op) Obj *op;
{
    static struct
    {
        char class;
 ...
 */
void radec2ha(np, ra, dec, hap) Now *np;
double ra, dec;
double *hap;
{
    double ha, lst;
 ...
 */
int lc(cx, cy, cw, x1, y1, x2, y2, sx1, sy1, sx2, sy2) int cx, cy, cw; /* circle bounding box corner and width */
 ...
 */
void hg_mag(h, g, rp, rho, rsn, mp) double h, g;
 ...
 * dimmer than fmag.
 */
int magdiam(fmag, magstp, scale, mag, size) int fmag; /* faintest mag */
 ...
 */
void gk_mag(g, k, rp, rho, mp) double g, k;
double rp;  /* sun-obj dist, AU */
double rho; /* earth-obj dist, AU */
double *mp;
{
 ...
 */
void solve_sphere(A, b, cc, sc, cap, Bp) double A, b;
double cc, sc;
double *cap, *Bp;
{
 ...
 */
matherr(xp) struct exception *xp;
{
    static char *names[8] = {"acos", "asin", "atan2", "pow", "exp", "log", "log10", "sqrt"};
    int i;
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: misc.c,v $ $Date: 2006/01/13 20:43:55 $ $Revision: 1.1.1.1 $ $Name:  $"};
