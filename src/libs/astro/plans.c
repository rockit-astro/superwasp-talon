 ...
 */
static void chap_trans(mjd, ret) double mjd; /* destination epoch */
 ...
 */
static void pluto_ell(mjd, ret) double mjd; /* epoch */
double *ret;                                /* ecliptic coordinates {l,b,r} at equinox of date */
{
    ...
    reduce_elements(mjdeq, mjd, degrad(inc0), degrad(omeg0), degrad(Om0), &inc, &omeg, &Om);
 ...
 */
static void planpos(mjd, obj, prec, ret) double mjd;
int obj;
double prec;
double *ret;
{
    if (mjd >= CHAP_BEGIN && mjd <= CHAP_END)
    {
        ...
        { /* VSOP for inner planets */
            vsop87(mjd, obj, prec, ret);
        }
    }
    else
    { /* outside Chapront time: */
        if (obj != PLUTO)
        { /* VSOP for all but Pluto */
            vsop87(mjd, obj, prec, ret);
        }
        else
        { /* Pluto mean elliptic orbit */
            pluto_ell(mjd, ret);
        }
    }
}
 ...
 */
void plans(mjd, p, lpd0, psi0, rp0, rho0, lam, bet, dia, mag) double mjd;
int p;
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: plans.c,v $ $Date: 2006/01/13 20:43:56 $ $Revision: 1.1.1.1 $ $Name:  $"};
