...
static void ecleq_aux P_((int sw, double mjd, double x, double y, double *p, double *q));
 ...
 * not included.
 */
void eq_ecl(mjd, ra, dec, lat, lng) double mjd;
double ra, dec;
double *lat, *lng;
{
 ...
 * not included.
 */
void ecl_eq(mjd, lat, lng, ra, dec) double mjd;
double lat, lng;
double *ra, *dec;
{
...
static void ecleq_aux(sw, mjd, x, y, p, q) int sw; /* +1 for eq to ecliptic, -1 for vv. */
double mjd;
...
double *p, *q; /* sw==1: p==lng, q==lat. sw==-1: p==ra, q==dec. */
{
    ...
    *p = atan(((sx * ceps) + (ty * seps * sw)) / cx);
    if (cx < 0)
        *p += PI; /* account for atan quad ambiguity */
    range(p, 2 * PI);
}

/* For RCS Only -- Do Not Edit */
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: eq_ecl.c,v $ $Date: 2006/01/13 20:43:54 $ $Revision: 1.1.1.1 $ $Name:  $"};
