 ...
 *   return -1
 */
int db_crack_line(s, op, whynot) char s[];
Obj *op;
char whynot[];
{
    ...
    case 'f': {
        ...
        (void)db_set_field(nf > 5 && flds[5][0] ? flds[5] : "2000", F_EPOCH, PREF_MDY, op);
        if (nf == 7)
    ...
    case 'e': {
        static int ids[] = {E_INC, E_LAN, E_AOP, E_A, E_N, E_E, E_M, E_CEPOCH, E_EPOCH, E_M1, E_M2};
        if (nf != 13 && nf != 14)
        {
            if (whynot)
    ...
    case 'E': {
        static int ids[] = {ES_EPOCH, ES_INC, ES_RAAN, ES_E, ES_AP, ES_M, ES_N, ES_DECAY, ES_ORBIT};
        if (nf != 11 && nf != 12)
        {
            if (whynot)
 ...
 */
static double tle_fld(l, from, thru) char *l;
int from, thru;
{
    char buf[32];
 ...
 */
static double tle_expfld(l, start) char *l;
int start;
{
    char buf[32];
    double v;
 ...
 */
int db_tle(name, l1, l2, op) char *name, *l1, *l2;
Obj *op;
{
    double ep;
    int i;
 ...
 */
void db_write_line(op, lp) Obj *op;
char *lp;
{
    int priorpref;
    int i;
    ...
    {
    case FIXED: {
        static int ids[] = {F_CLASS, F_SPECT, F_RA, F_DEC, F_MAG, F_EPOCH, F_SIZE};
    ...
    case ELLIPTICAL: {
        static int ids[] = {E_INC, E_LAN, E_AOP, E_A, E_N, E_E, E_M, E_CEPOCH, E_EPOCH, E_M1, E_M2, E_SIZE};
    ...
    case HYPERBOLIC: {
        static int ids[] = {H_EP, H_INC, H_LAN, H_AOP, H_E, H_QP, H_EPOCH, H_G, H_K, H_SIZE};
    ...
    case PARABOLIC: {
        static int ids[] = {P_EP, P_INC, P_AOP, P_QP, P_LAN, P_EPOCH, P_G, P_K, P_SIZE};
    ...
    case EARTHSAT: {
        static int ids[] = {ES_EPOCH, ES_INC, ES_RAAN, ES_E, ES_AP, ES_M, ES_N, ES_DECAY, ES_ORBIT, ES_DRAG};
 ...
 */
int db_set_field(bp, id, pref, op) char bp[];
int id;
PrefDateFormat pref;
Obj *op;
{
 ...
 */
int get_fields(s, delim, fields) char *s;
int delim;
char *fields[];
{
    int n;
 ...
 */
int db_chk_planet(name, op) char name[];
Obj *op;
{
    char namecpy[256];
    int i;
 ...
 * attempt, else -1.
 */
static int line_candidate(buf) char *buf;
{
    char c = buf[0];
 ...
 */
static void crack_year(bp, pref, p) char *bp;
PrefDateFormat pref;
double *p;
{
    int m, y;
 ...
 */
static int db_get_field(op, id, bp) Obj *op;
int id;
char *bp;
{
    char *bpsave = bp;
    ...
    case F_SIZE:
        ...
        {
            sprintf(bp, "|%g|%g", op->f_size * get_ratio(op), raddeg(get_pa(op)));
            bp += strlen(bp);
        }
        break;
    case F_CLASS:
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: dbfmt.c,v $ $Date: 2006/01/13 20:43:54 $ $Revision: 1.1.1.1 $ $Name:  $"};
