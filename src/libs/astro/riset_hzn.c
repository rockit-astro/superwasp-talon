/* find rise and set circumstances and related functions. */
...
static int find_max P_((Now * np, Obj *op, double tr, double ts, double *tp, double *alp));

static double hzn0 P_((void));

/* find where and when an object, op, will rise and set and it's transit
 *   circumstances. all times are utc mjd, angles rads e of n.
 * hznf is a pointer to a function that, when called with azimuth in rads E
 *   of N, returns the altitude of the horizon in rads above horizontal.
 *   if passed as NULL, 0 alt will be assumed for all az.
 */
void riset_hzn(np, op, hznf, rp) Now *np;
Obj *op;
double (*hznf)();
RiseSet *rp;
{
    ...
    int rss;       /* temp status */
    double az;     /* az under test */

    /* default horizon is 0 if nothing provided otherwise */
    if (!hznf)
        hznf = hzn0;
    ...
    /* start with ephemeride of object at noon.
     * N.B. this is the only time we check for failure from obj_cir
     */
    ...
    /* make initial guess using alt at N or S horizon (arbitrarily) */
    riset(o.s_gaera, o.s_gaedec, lat, (*hznf)(az = 0.), &lr, &ls, &ar, &as, &rss);
    if (rss == 1) /* no rise */
        riset(o.s_gaera, o.s_gaedec, lat, (*hznf)(az = PI), &lr, &ls, &ar, &as, &rss);
    switch (rss)
    {
    case 0:
        break;
    ...
    /* now we know it can rise at the alt @ az anyway */
    n.n_mjd = mjdn + (lr - lstn) / SIDRATE;
    findRise(&n, &o, mjdn, hznf);
}
 ...
 */
void riset_cir(np, op, dis, rp) Now *np;
Obj *op;
double dis;
RiseSet *rp;
{
 ...
 */
void twilight_cir(np, dis, dawn, dusk, status) Now *np;
double dis;
double *dawn, *dusk;
int *status;
{
 ...
 */
static void e_riset_cir(np, op, dis, rp) Now *np;
Obj *op;
double dis;
RiseSet *rp;
{
 ...
 */
static int find_0alt(dt, dis, np, op) double dt; /* hours from noon to first guess at event */
 ...
 */
static int find_transit(dt, np, op) double dt;
Now *np;
Obj *op;
{
#define MAXLOOPS 10
 ...
 */
static int find_max(np, op, tr, ts, tp, alp) Now *np;
Obj *op;
...
/* dummy horizon function that always returns 0 */
static double hzn0()
{
    return (0.);
}
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: riset_hzn.c,v $ $Date: 2006/01/13 20:43:56 $ $Revision: 1.1.1.1 $ $Name:  $"};
