 ...
 *	60:	<w>:mm
 */
void fs_sexa(out, a, w, fracbase) char *out;
double a;
int w;
int fracbase;
{
...
double jd;
{
    ...
    if ((d < 1.0 && d - floor(d) >= .9999995) || (d < 10.0 && d - floor(d) >= .999995) ||
 ...
 */
void f_scansex(o, bp, np) double o; /* old value */
 ...
 */
void f_sscandate(bp, pref, m, d, y) char *bp;
...
double *d;
{
     ...
     */
    if (ncomp == 1 && (ndp > 0 || (pref == PREF_MDY && !(comp[0] >= 1 && comp[0] <= 12)) ||
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: formats.c,v $ $Date: 2006/01/13 20:43:54 $ $Revision: 1.1.1.1 $ $Name:  $"};
