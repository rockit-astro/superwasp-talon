...
static void ab_aux P_((double mjd, double *x, double *y, double lsn, int mode));
 ...
 * lsn (in radians)
 */
void ab_ecl(mjd, lsn, lam, bet) double mjd, lsn, *lam, *bet;
{
 ...
 * lsn (in radians)
 */
void ab_eq(mjd, lsn, ra, dec) double mjd, lsn, *ra, *dec;
{
 ...
 */
static void ab_aux(mjd, x, y, lsn, mode) double mjd, *x, *y, lsn;
int mode;
{
    ...
    case AB_ECL_EOD: /* ecliptical coords */
    {
        ...
        *lam -= ABERR_CONST / cos(*bet) * (cos(dlsun) - eexc * cos(dlperi));
        *bet -= ABERR_CONST * sin(*bet) * (sin(dlsun) - eexc * sin(dlperi));
    }
    break;
    ...
    case AB_EQ_EOD: /* equatoreal coords */
    {
        ...
        dra = ABERR_CONST / cd * (-(cr * cls * ce + sr * sls) + eexc * (cr * cp * ce + sr * sp));

        ddec = se / ce * cd - sr * sd; /* tmp use */
        ddec = ABERR_CONST * (-(cls * ce * ddec + cr * sd * sls) + eexc * (cp * ce * ddec + cr * sd * sp));
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: aberration.c,v $ $Date: 2006/01/13 20:43:53 $ $Revision: 1.1.1.1 $ $Name:  $"};
