...
static void precess_hiprec P_((double mjd1, double mjd2, double *ra, double *dec));
 ...
 */
void precess(mjd1, mjd2, ra, dec) double mjd1, mjd2; /* initial and final epoch modified JDs */
 ...
 */
static void precess_hiprec(mjd1, mjd2, ra, dec) double mjd1, mjd2; /* initial and final epoch modified JDs */
double *ra, *dec;                                                  /* ra/dec for mjd1 in, for mjd2 out */
{
    ...
    if (fabs(from_equinox - 2000.0) > .02)
    {
        ...
        B = DCOS(alpha_in - z_A) * DCOS(theta_A) * DCOS(delta_in) + DSIN(theta_A) * DSIN(delta_in);
        C = -DCOS(alpha_in - z_A) * DSIN(theta_A) * DCOS(delta_in) + DCOS(theta_A) * DSIN(delta_in);
    ...
    if (fabs(to_equinox - 2000.0) > .02)
    {
        ...
        B = DCOS(alpha2000 + zeta_A) * DCOS(theta_A) * DCOS(delta2000) - DSIN(theta_A) * DSIN(delta2000);
        C = DCOS(alpha2000 + zeta_A) * DSIN(theta_A) * DCOS(delta2000) + DCOS(theta_A) * DSIN(delta2000);
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: precess.c,v $ $Date: 2006/01/13 20:43:56 $ $Revision: 1.1.1.1 $ $Name:  $"};
