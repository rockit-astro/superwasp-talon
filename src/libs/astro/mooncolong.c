...
static void Librations P_((double RAD, double LAMH, double BH, double OM, double F, double L, double L1, double *L0,
                           double *B0));
static void Moon P_((double RAD, double T, double T2, double LAM0, double R, double M, double *F, double *L1,
                     double *OM, double *LAM, double *B, double *DR, double *LAMH, double *BH));
static void Sun P_((double RAD, double T, double T2, double *L, double *M, double *R, double *LAM0));
 ...
 */
void moon_colong(jd, lt, lg, cp, kp, ap, sp) double jd; /* jd */
...
static void Librations(RAD, LAMH, BH, OM, F, L, L1, L0, B0) double RAD;
double LAMH;
double BH;
double OM;
double F;
...
double *B0;
{
    ...
    PSI = -17.2 * sin(OM) - 1.32 * sin(2 * L) - .23 * sin(2 * L1) + .21 * sin(2 * OM);
    PSI = PSI * RAD / 3600;
...
static void Moon(RAD, T, T2, LAM0, R, M, F, L1, OM, LAM, B, DR, LAMH, BH) double RAD;
double T;
double T2;
double LAM0;
double R;
...
double *BH;
{
    ...
    *DR = 385000 + SUMR;
    ...
    /* geocentric latitude */
    *B = 5.128 * sin(*F) + .2806 * sin(M1 + *F) + .2777 * sin(M1 - *F) + .1732 * sin(D2 - *F);
    SUML = 6.289 * sin(M1) + 1.274 * sin(D2 - M1) + .6583 * sin(D2) + .2136 * sin(2 * M1) - .1851 * sin(M) -
           .1143 * sin(2 * *F);
    *LAM = *L1 + SUML * RAD;
    DIST = *DR / R;
    *LAMH = (LAM0 + 180 + DIST * cos(*B) * sin(LAM0 * RAD - *LAM) / RAD) * RAD;
    *BH = DIST * *B * RAD;
}

static void Sun(RAD, T, T2, L, M, R, LAM0) double RAD;
double T;
double T2;
double *L;
double *M;
...
double *LAM0;
{
    ...
    *M = *M * RAD;
    ...
    C = (1.915 - .004817 * T - .000014 * T2) * sin(*M) + (.01999 - .000101 * T) * sin(2 * *M) + .00029 * sin(3 * *M);
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: mooncolong.c,v $ $Date: 2006/01/13 20:43:55 $ $Revision: 1.1.1.1 $ $Name:  $"};
