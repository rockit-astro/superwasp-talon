...
void sdp4(sat, pos, dpos, TSINCE) SatData *sat;
Vec3 *pos;
Vec3 *dpos;
double TSINCE;
{
    ...
    double A1, A3OVK2, AO, C2, COEF, COEF1, DEL1, DELO, EETA, ETA, ETASQ, PERIGE, PINVSQ, PSISQ, QOMS24, S4, THETA4,
        TSI, X1M5TH, XHDOT1;

    /* private temporary variables */
    double A, AXN, AYN, AYNL, BETA, BETAL, CAPU, COS2U, COSEPW, COSIK, COSNOK, COSU, COSUK, E, ECOSE, ELSQ,
        EM = 0, EPW, ESINE, OMGADF, PL, R, RDOT, RDOTK, RFDOT, RFDOTK, RK, SIN2U, SINEPW, SINIK, SINNOK, SINU, SINUK,
        TEMP, TEMP1, TEMP2, TEMP3, TEMP4, TEMP5, TEMP6, TEMPA, TEMPE, TEMPL, TSQ, U, UK, UX, UY, UZ, VX, VY, VZ,
        XINC = 0, XINCK, XL, XLL, XLT, XMAM, XMDF, XMX, XMY, XN, XNODDF, XNODE, XNODEK;
    ...
    if (!sat->prop.sdp4)
    {
        ...
        C2 = COEF1 * XNODP *
             (AODP * (1.0 + 1.5 * ETASQ + EETA * (4.0 + ETASQ)) +
              .75 * CK2 * TSI / PSISQ * X3THM1 * (8.0 + 3.0 * ETASQ * (8.0 + ETASQ)));
        C1 = BSTAR * C2;
        SINIO = sin(XINCL);
             ...
             (ETA * (2.0 + .5 * ETASQ) + EO * (.5 + 2.0 * ETASQ) -
              2.0 * CK2 * TSI / (AODP * PSISQ) *
                  (-3.0 * X3THM1 * (1.0 - 2.0 * EETA + ETASQ * (1.5 - .5 * EETA)) +
                   .75 * X1MTH2 * (2.0 * ETASQ - EETA * (1.0 + ETASQ)) * cos(2.0 * OMEGAO)));
        THETA4 = THETA2 * THETA2;
        TEMP1 = 3.0 * CK2 * PINVSQ * XNODP;
        TEMP2 = TEMP1 * CK2 * PINVSQ;
        TEMP3 = 1.25 * CK4 * PINVSQ * PINVSQ * XNODP;
        XMDOT = XNODP + 0.5 * TEMP1 * BETAO * X3THM1 + .0625 * TEMP2 * BETAO * (13.0 - 78.0 * THETA2 + 137.0 * THETA4);
        X1M5TH = 1.0 - 5.0 * THETA2;
        OMGDOT = -.5 * TEMP1 * X1M5TH + .0625 * TEMP2 * (7.0 - 114.0 * THETA2 + 395.0 * THETA4) +
                 TEMP3 * (3.0 - 36.0 * THETA2 + 49.0 * THETA4);
        XHDOT1 = -TEMP1 * COSIO;
        XNODOT = XHDOT1 + (.5 * TEMP2 * (4.0 - 19.0 * THETA2) + 2.0 * TEMP3 * (3.0 - 7.0 * THETA2)) * COSIO;
        ...
        printf("%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n", EOSQ, SINIO, COSIO, BETAO, AODP, THETA2, SING,
               COSG, BETAO2, XMDOT, OMGDOT, XNODOT, XNODP);
#endif
        dpinit(sat, EOSQ, SINIO, COSIO, BETAO, AODP, THETA2, SING, COSG, BETAO2, XMDOT, OMGDOT, XNODOT, XNODP);
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: sdp4.c,v $ $Date: 2006/01/13 20:43:57 $ $Revision: 1.1.1.1 $ $Name:  $"};
