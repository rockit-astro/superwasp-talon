 ...
 */
void utc_gst(mjd, utc, gst) double mjd;
double utc;
double *gst;
{
 ...
 * N.B. mjd must be at the beginning of the day.
 */
void gst_utc(mjd, gst, utc) double mjd;
double gst;
double *utc;
{
 ...
 */
static double gmst0(mjd) double mjd; /* date at 0h UT in julian days since MJD0 */
{
    double T, x;
...
static double tnaught(mjd) double mjd; /* julian days since 1900 jan 0.5 */
{
    double dmjd;
    int m, y;
    double d;
    ...
    t0 = 6.57098e-2 * (mjd - dmjd) -
         (24 - (6.6460656 + (5.1262e-2 + (t * 2.581e-5)) * t) - (2400 * (t - (((double)y - 1900) / 100))));
    range(&t0, 24.0);
    return (t0);
}
