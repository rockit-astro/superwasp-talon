 ...
 * radians.
 */
void eq_gal(mjd, ra, dec, lat, lng) double mjd, ra, dec;
double *lat, *lng;
{
    galeq_init();
 ...
 * given epoch.
 */
void gal_eq(mjd, lat, lng, ra, dec) double mjd, lat, lng;
double *ra, *dec;
{
    galeq_init();
...
static void galeq_aux(sw, x, y, p, q) int sw; /* +1 for eq to gal, -1 for vv. */
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: eq_gal.c,v $ $Date: 2006/01/13 20:43:54 $ $Revision: 1.1.1.1 $ $Name:  $"};
