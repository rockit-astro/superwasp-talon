 ...
 */
int serv_conn(char *name, int fd[2], char msg[])
{
    char ws[1024];
 ...
 * return 0 if ok, else -1 with errno set and an excuse in msg[].
 */
int cli_conn(char *name, int fd[2], char msg[])
{
    char ws[1024];
    struct stat ss;
...
void dis_conn(char *name, int fd[2])
{
    char ws[1024];
 ...
 */
int serv_write(int fd[2], int code, char *msg, char *err)
{
    char buf[1024];
 ...
 * return 0 if ok, else fill err[] with excuse and return -1.
 */
int cli_write(int fd[2], char *msg, char *err)
{
    int l = strlen(msg);
    int s;
 ...
 */
int serv_read(int fd[2], char *buf, int bufl)
{
    int found;
    int n;
    int s;
 ...
 * else fill buf[] with excuse and return -1.
 */
int cli_read(int fd[2], int *code, char *buf, int bufl)
{
    int n, v;
    char *sp;
    int found;
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: cliserv.c,v $ $Date: 2006/01/13 20:44:02 $ $Revision: 1.1.1.1 $ $Name:  $"};
