 ...
 */
int photStdRead(FILE *fp, PStdStar **spp)
{
    char buf[1024];
    PStdStar *sp;
    int nsp;
...
void photFree(PStdStar *sp, int n)
{
    PStdStar *spsave;
 ...
 */
int photReadCalConst(FILE *fp, double jd, double jdwin, PCalConst *cp, double *jdp, char *msg)
{
    long best_where;
    double best_jd;
    char buf[1024];
    ...
    if (fgets(buf, sizeof(buf), fp) == NULL ||
        sscanf(buf, "B: %lf %lf %lf %lf %lf %lf", &cp->BV0, &cp->BV0e, &cp->Bkp, &cp->Bkpe, &cp->Bkpp, &cp->Bkppe) != 6)
    {
    ...
    /* read V line */
    if (fgets(buf, sizeof(buf), fp) == NULL ||
        sscanf(buf, "V: %lf %lf %lf %lf %lf %lf", &cp->VV0, &cp->VV0e, &cp->Vkp, &cp->Vkpe, &cp->Vkpp, &cp->Vkppe) != 6)
    {
        (void)sprintf(msg, "Bad line: %s", buf);
        return (-1);
    }
    ...
    /* read R line */
    if (fgets(buf, sizeof(buf), fp) == NULL ||
        sscanf(buf, "R: %lf %lf %lf %lf %lf %lf", &cp->RV0, &cp->RV0e, &cp->Rkp, &cp->Rkpe, &cp->Rkpp, &cp->Rkppe) != 6)
    {
        (void)sprintf(msg, "Bad line: %s", buf);
        return (-1);
    }
    ...
    /* read I line */
    if (fgets(buf, sizeof(buf), fp) == NULL ||
        sscanf(buf, "I: %lf %lf %lf %lf %lf %lf", &cp->IV0, &cp->IV0e, &cp->Ikp, &cp->Ikpe, &cp->Ikpp, &cp->Ikppe) != 6)
    {
        (void)sprintf(msg, "Bad line: %s", buf);
        return (-1);
    }
 ...
 */
int photReadDefCalConst(FILE *fp, PCalConst *cp, double *Bp, double *Vp, double *Rp, double *Ip, char *msg)
{
    char buf[1024];
    int ok;
    ...
    while (fgets(buf, sizeof(buf), fp))
    {
        if (sscanf(buf, "B: %lf %lf %lf %lf", Bp, &cp->BV0, &cp->Bkp, &cp->Bkpp) == 4)
        {
            ok = 1;
            break;
        }
    ...
    if (ok)
        if (!fgets(buf, sizeof(buf), fp) || sscanf(buf, "V: %lf %lf %lf %lf", Vp, &cp->VV0, &cp->Vkp, &cp->Vkpp) != 4)
            ok = 0;

    if (ok)
        if (!fgets(buf, sizeof(buf), fp) || sscanf(buf, "R: %lf %lf %lf %lf", Rp, &cp->RV0, &cp->Rkp, &cp->Rkpp) != 4)
            ok = 0;

    if (ok)
        if (!fgets(buf, sizeof(buf), fp) || sscanf(buf, "I: %lf %lf %lf %lf", Ip, &cp->IV0, &cp->Ikp, &cp->Ikpp) != 4)
            ok = 0;
 ...
 */
static int addStar(PStdStar *sp, char buf[])
{
    double rh, rm, rs;
    double dd, dm, ds;
    double v, bv, vr, vi;
    ...
    n = sscanf(buf, "%s %lf %lf %lf%[ -] %lf %lf %lf %lf %lf %lf %lf", name, &rh, &rm, &rs, dsign, &dd, &dm, &ds, &v,
               &bv, &vr, &vi);
    if (n != 12)
        return (-1);
