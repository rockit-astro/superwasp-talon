 ...
 */
int csimcd_saccept(int serv_fd)
{
 ...
 */
int csimcd_clconn(char *host, int port)
{
    struct sockaddr_in cli_socket;
    struct hostent *hp;
    int cli_fd, len;
...
static void fdiAdd(int fd, int haddr, int naddr, int why)
{
    FDInfo *fp, *lfp;
...
static int common_open(char *host, int port, int addr, OpenWhy why, int client)
{
    Byte preamble[3];
    int fd;
 ...
 * return fd or -1.
 */
int csi_open(char *host, int port, int addr)
{
 ...
 * return fd or -1.
 */
int csi_sopen(char *host, int port, int addr, int baud)
{
 ...
 * return fd or -1.
 */
int csi_bopen(char *host, int port, int addr)
{
 ...
 * or -1 if error.
 */
int csi_r(int fd, char buf[], int buflen)
{
    int s, n;
 ...
 */
int csi_wr(int fd, char rbuf[], int rbuflen, char *fmt, ...)
{
    va_list ap;
    char wbuf[1024];
    int l;
