...
IniList *iniRead(char *fn)
{
    ...
    while (fgets(buf, sizeof(buf), fp))
    {
        ...
        if (sscanf(bp, "%[^= \t] = %s", name, valu) == 2)
        {
            ...
            listp->list = (char **)realloc((char *)listp->list, (++listp->nlist) * sizeof(char *));
 ...
 */
char *iniFind(IniList *listp, char *section, char *name)
{
    char full[1024];
    int nfull;
    char **ep;
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: crackini.c,v $ $Date: 2006/01/13 20:44:03 $ $Revision: 1.1.1.1 $ $Name:  $"};
