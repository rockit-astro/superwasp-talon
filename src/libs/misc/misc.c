...
void hdRange(double *hap, double *decp)
{
    rdRange(hap, decp);
    haRange(hap);
}
...
void rdRange(double *rap, double *decp)
{
    if (*decp > PI / 2)
    {
        *decp = PI - *decp;
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: misc.c,v $ $Date: 2006/01/13 20:44:03 $ $Revision: 1.1.1.1 $ $Name:  $"};
