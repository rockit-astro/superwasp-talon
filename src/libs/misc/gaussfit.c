...
static double g_chisqr(p) double p[];
{
    double max = p[0];
    double cen = p[1];
    double sig = p[2];
...
void gaussfit(pix, n, gmaxp, cenp, fwhmp) int pix[]; /* pixels along a line */
int n;                                               /* n pixels */
...
int main(int ac, char *av[])
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        printf("x = %6.2f y = %6.2f f() = %6.2f\n", (double)i, (double)pix[i],
               gmax * exp(-(dx * dx) / (2 * sig * sig)));
    }
...
static char *rcsid[2] = {(char *)rcsid,
                         "@(#) $RCSfile: gaussfit.c,v $ $Date: 2006/01/13 20:44:03 $ $Revision: 1.1.1.1 $ $Name:  $"};
