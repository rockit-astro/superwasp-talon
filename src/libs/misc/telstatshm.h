...
#define FOCUS_READY (!telstatshmp->minfo[TEL_OM].have || telstatshmp->minfo[TEL_OM].cvel == 0)

#define ANY_HOMING                                                                                                     \
    (telstatshmp->minfo[TEL_HM].homing || telstatshmp->minfo[TEL_DM].homing || telstatshmp->minfo[TEL_RM].homing ||    \
     telstatshmp->minfo[TEL_OM].homing)

#define ANY_LIMITING                                                                                                   \
    (telstatshmp->minfo[TEL_HM].limiting || telstatshmp->minfo[TEL_DM].limiting ||                                     \
     telstatshmp->minfo[TEL_RM].limiting || telstatshmp->minfo[TEL_OM].limiting)
...
/* telaxes.c */
extern void tel_hadec2xy(double H, double D, TelAxes *tap, double *X, double *Y);
extern void tel_xy2hadec(double X, double Y, TelAxes *tap, double *H, double *D);
extern void tel_hadec2PA(double H, double D, TelAxes *tap, double latitude, double *PA);
