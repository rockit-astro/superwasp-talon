...
/* cross-check */
static void
prChk (TelAxes *tap, double H, double D, double X, double Y)
{
	double x, y, h, d; 
	double ca;
	...
	solve_sphere (X-x, PI/2-(Y+tap->YC), sin(y+tap->YC),
						cos(y+tap->YC), &ca, NULL);
	printf (" %7.1f", 3600*raddeg(acos(ca)));
...
int
tel_solve_axes (
double H[], 	/* HA of the known stars */
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: telaxes.c,v $ $Date: 2006/01/13 20:44:04 $ $Revision: 1.1.1.1 $ $Name:  $"};
