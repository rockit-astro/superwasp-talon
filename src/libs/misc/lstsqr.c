...
static int amoeba(double **p, double *y, int ndim, double ftol, double (*funk)(), int *nfunk);

/* this lets us map 1-based arrays into 0-based arrays */
static double (*chisqr_0based)(double p[]);
static double chisqr_1based(double p[])
{
 ...
 */
int lstsqr(double (*chisqr)(double p[]), /* function to evaluate chisqr with at p */
...
static double amotry(double **p, double *y, double *psum, int ndim, double (*funk)(), int ihi, int *nfunk, double fac);
...
static int amoeba(p, y, ndim, ftol, funk, nfunk) double **p, y[], ftol, (*funk)();
int ndim, *nfunk;
{
...
static double amotry(p, y, psum, ndim, funk, ihi, nfunk, fac) double **p, *y, *psum, (*funk)(), fac;
int ndim, ihi, *nfunk;
{
    int j;
...
static void nrerror(error_text) char error_text[];
{
...
static double *vector(nl, nh) int nl, nh;
{
    double *v;
...
static void free_vector(v, nl, nh) double *v;
int nl, nh;
{
    free((char *)(v + nl));
}
