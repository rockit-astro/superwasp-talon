 ...
 */
void catalogBrowse(Widget top_w, CatBrowseCB cbp)
{
    toplevel_w = top_w;
    callback = cbp;
...
static void list_edb(FILE *fp)
{
    ...
    while (fgets(buf, sizeof(buf), fp) != NULL)
    {
        ...
        if (db_crack_line(buf, &o, NULL) == 0)
        {
            ...
            names = (XmString *)XtRealloc((void *)names, (nnames + 1) * sizeof(XmString));
            names[nnames++] = name;
        }
    }
 ...
 */
static void list_select_cb(Widget w, XtPointer client, XtPointer data)
{
...
static void fsb_ok_cb(Widget w, XtPointer client, XtPointer data)
{
    XmFileSelectionBoxCallbackStruct *s = (XmFileSelectionBoxCallbackStruct *)data;
    char *sp;
    FILE *fp;
...
static void fsb_cancel_cb(Widget w, XtPointer client, XtPointer data)
{
    XtUnmanageChild(main_w);
}

/* create the main form dialog */
static void create_main_w()
{
    Widget w;
    Arg args[20];
    int n;
    ...
    n = 0;
    XtSetArg(args[n], XmNtitle, "Catalog Browser");
    n++;
    ...
    n++;
    XtSetArg(args[n], XmNbottomWidget, fsb_w);
    n++;
    XtSetArg(args[n], XmNleftAttachment, XmATTACH_FORM);
    n++;
    XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
...
static void newdb_cb(char *edb_entry)
{
    printf("Selection: %s\n", edb_entry);
}

static void browse_cb(Widget w, XtPointer client, XtPointer data)
{
