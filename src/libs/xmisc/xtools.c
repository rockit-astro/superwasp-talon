...
raiseShell (Widget w)
{
	...
	if (move_ok) {
	    ...
	    XtVaSetValues (shell,
		XmNx, ((tx/scrw)*scrw) + (sx%scrw) - 10*scrw,
		XmNy, ((ty/scrh)*scrh) + (sy%scrh) - 10*scrh,
		NULL);
	}
 ...
 */
char *
getXRes (Widget any, char *name, char *def)
{
...
void
stopchk_up(Widget shell_w, char *title, char *message)
{
	if (!stopchk_w) {
	    Arg args[20];
	    Colormap cm;
	    ...
	    n = 0;
	    XtSetArg(args[n], XmNcolormap, cm);  n++;
	    ...
	    XtUnmanageChild(XmMessageBoxGetChild(stopchk_w,
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: xtools.c,v $ $Date: 2006/01/13 20:43:52 $ $Revision: 1.1.1.1 $ $Name:  $"};
